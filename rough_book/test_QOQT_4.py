...
def plot_optimal(psi_initial,res):
    ...
    plt.title("Wigner function of the OPTIMIZED final state with fidelity= %f for alpha 1=%f, SNAP n=%d, SNAP theta=%f, alpha 2=%f in 3D"%(1-(res['fun'][0]),alpha_1,n_SNAP,theta_SNAP,alpha_2))
    plot_wigner(optimized_psi_final,colorbar=True, projection='2d')
    plt.title("Wigner function of the OPTIMIZED final state with fidelity= %f for alpha 1=%f, SNAP n=%d, SNAP theta=%f, alpha 2=%f in 2D"%(1-(res['fun'][0]),alpha_1,n_SNAP,theta_SNAP,alpha_2))
    plt.show()
...
implicit_fidelity=0.90
implicit_infidelity=0.1
...
for i in n_SNAP_enumeration:
    ...
    for j in alpha_enumeration:
        x0=np.array([j[0],theta_SNAP,int(i),j[1]])
...
if len(result_list)!=0:
    print("The Optimized Outputs reaching the implicit state fidelity threshold of 0.9 are:")
    for i in range(len(result_list)):
        print(result_list[i][1])
        print("\n")
        plot_optimal(psi_initial,result_list[i][1]) #plot_optimal(psi_initial,res)
else:
    print("Proper Optimization NOT ACHIEVED. Output states fidelities less than the implicit state fidelity threshold of 0.9...")
    print("Please revise your initial guess of the gate sequence parameters or change the tolerances")
